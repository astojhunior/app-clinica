@php
    $user  = auth()->user();
    $cargo = $user?->empleado?->cargo?->DescripcionCargo;
@endphp
            ...
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}">
                        <img
                            src="{{ asset('images/login-medicos01.png') }}"
                            alt="Clínica Calidad"
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    {{-- Dashboard: todos los que tengan cargo --}}
                    @if($cargo)
                    ...
                    {{-- Pacientes: Recepcionista o Admin TI --}}
                    @if(in_array($cargo, ['Recepcionista', 'Administrador TI']))
                        <x-nav-link :href="route('pacientes.index')" :active="request()->routeIs('pacientes.*')">
                            {{ __('👥 Pacientes') }}
                        </x-nav-link>
                    @endif

                    {{-- Empleados: solo Admin TI --}}
                    @if($cargo === 'Administrador TI')
                        <x-nav-link :href="route('empleados.index')" :active="request()->routeIs('empleados.*')">
                            {{ __('👨‍⚕️ Empleados') }}
                        </x-nav-link>
                    @endif

                    {{-- Citas: Recepcionista o Admin TI --}}
                    @if(in_array($cargo, ['Recepcionista', 'Administrador TI']))
                    ...
                    {{-- Pagos: Finanzas o Admin TI --}}
                    @if(in_array($cargo, ['Personal de Facturación y Cobranza', 'Administrador TI']))
                        <x-nav-link :href="route('pagos.index')" :active="request()->routeIs('pagos.*')">
                            {{ __('💰 Pagos') }}
                        </x-nav-link>
                    @endif

                    {{-- Consultas: Doctor o Admin TI --}}
                    @if(in_array($cargo, ['Doctor', 'Administrador TI']))
